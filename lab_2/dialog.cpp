    ...
    , ui(new Ui::Dialog)
{
    ...
    ui->database_name ->setText(settings.value("databasename").toString());
...
void Dialog::save_db_options()
{
    ...
    settings.setValue("databasename", ui->database_name   ->text());
    settings.setValue("port"        , ui->port            ->text());
    settings.setValue("username"    , ui->user_name       ->text());
